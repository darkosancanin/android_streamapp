...
public class SeparatedListAdapter extends BaseAdapter implements SectionIndexer {
    ...
    public void addSection(String tag, String title, Adapter adapter) {
        final SectionHeader header = new SectionHeader(tag, title,
                adapter);
        adapter.registerDataSetObserver(new SectionDataSetObserver(
                header));
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        for (final SectionHeader section : this .sections) {
            ...
            // check if position inside this section
            if (position == 0) {
                return headersAdapter.getView(sectionnum, convertView,
                        parent);
            }
            if (position < size) {
    ...
    public SectionAndRow getSectionAndRowForPosition(int position) {
        int i = 0;
        for (final SectionHeader section : this .sections) {
            final int size = section.sectionCount;

            if (position == 0) {
                return new SectionAndRow(0,0);
            }
            if (position <= size) {
                return new SectionAndRow(i, position - 1);
            }
